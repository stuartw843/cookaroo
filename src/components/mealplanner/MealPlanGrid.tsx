...
export const MealPlanGrid: React.FC<MealPlanGridProps> = ({
  ...
  const mealTypeHasItems = (mealTypeKey: string) => {
    return weekDays.some((_, dayIndex) => getMealItem(dayIndex, mealTypeKey))
  }
  ...
  const MealSlot: React.FC<{
    ...
    if (!editMode && !item) {
      return null // Don't show empty slots in view mode
    }
      ...
      <CardContent>
        ...
        <div className="block lg:hidden space-y-4">
          {weekDays.map((day, dayIndex) => {
            // Check if this day has any meals to show
            const dayMeals = visibleMealTypes.map(mealType => ({
              mealType,
              item: getMealItem(dayIndex, mealType.key)
            })).filter(({ item, mealType }) => editMode || item)
                ...
                {visibleMealTypes.map(mealType => (
                  <div key={mealType.key} className="grid grid-cols-8 gap-3">
                    ...
                    {weekDays.map((_, dayIndex) => {
                      const item = getMealItem(dayIndex, mealType.key)
                      return (
                        <MealSlot
